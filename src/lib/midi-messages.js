// midi-messages.js
import { MIDI_CHANNEL_IN, NOTE_OFF, NOTE_ON, EVENT_NOTE_START, EVENT_NOTE_STOP } from './constants';

export function startMidiNote ( pitch, velocity, selectedOutput, midiChannel = MIDI_CHANNEL_IN ) {
    console.log( "START MIDI NOTE", pitch, velocity );
    selectedOutput.send( [ 0x90 + ( midiChannel - 1 ), pitch, velocity ] );
}

export function stopMidiNote ( pitch, selectedOutput, midiChannel = MIDI_CHANNEL_IN ) {
